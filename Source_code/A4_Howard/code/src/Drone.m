classdef Drone < handle
    ...
    methods 
        ...
        %
        function obj = Drone(radius, pos, type, c_d_pot_c, c_d_pot_out_r_c)
            obj.radius = radius;
            obj.pos = pos;
            obj.type = 1;
            obj.dim = length(pos);
            obj.c_d_pot_c = c_d_pot_c;
            obj.c_d_pot_out_r_c = c_d_pot_out_r_c;
    ...
    methods %(Access = private)
        ...
        function val = GetCenterPotential( obj, r ) 
            ...
            % becomes a minimum
            R = obj.radius; 
            d_pot_c = obj.c_d_pot_c * 1e3; %(CJ)
            d_pot_out_r_c = obj.c_d_pot_out_r_c * 10; %(CJ) 
            
            if( r < R )
                val = d_pot_c*exp( -(r/R)^2 );
            else
                val = d_pot_out_r_c*(r-(R+.3) )^2; % Initial value for potential
            end
