function table = gen_next_seed(seed_pool_txt, option) %Nd = 4 as example
    ...
    if strcmpi(option,'big')
        len_rand = alpha_big + beta_big * rand();
        theta_rand1 = 2*pi()*rand();
        theta_rand2 = 2*pi()*rand();
    else
        len_rand = alpha_small + beta_small * rand();
        theta_rand1 = 2*pi()*rand();
        theta_rand2 = 2*pi()*rand();
    end
    ...
    temp_len_mid = len_rand * cos(theta_rand2);
    new_coor4 = [temp_len_mid * cos(theta_rand1), temp_len_mid * sin(theta_rand1), len_rand * sin(theta_rand2)];
    
    %... add more for additional agent
        
    table = [temp_table(1:3)'+new_coor1,temp_table(4:6)'+new_coor2,temp_table(7:9)'+new_coor3, temp_table(10:12)'+new_coor4];
