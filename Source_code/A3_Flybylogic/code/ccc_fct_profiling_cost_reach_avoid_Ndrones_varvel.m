function [negative_rob,xx,yy,zz] = ccc_fct_profiling_cost_reach_avoid_Ndrones_varvel(var,optParams, c_C, c_C1, c_C2, c_Ib, c_Jb, c_Ja, c_Ia)
import casadi.*
type_of = isfloat(var); %0 for casadi
w = var(1:numel(var)/2);
v = var(numel(var)/2+1:end);
N_per_T = round(optParams.N_per_T);
C = c_C * 50.0; %const for smooth min/max operation %for 2 drones, use 10 with a period of 5s, casadi is unstable numerically
C1 = c_C1 * 50.0; %const for smooth max %20 works for 10 drones, det init points
C2 = c_C2 * 50.0; %5 makes no numerical instblty in 12 drones
...
%     rho_goal(d) = alwaysEventually(states, optParams.goal.poly, I, J, C);
    ...
    %
    % check that I+J <= H_formula specified in the head file!
    Ia = c_Ia * 1;%0; % in seconds
    Ib = c_Ib * 5; % also seconds
    Ja = 0;%c_Ja * 1;%0 % seconds
    Jb = c_Jb * 1;
    I = Ia*numel(dT):Ib*numel(dT); % convertation to time-steps (needed for MTL formulas)
    J = Ja*numel(dT):Jb*numel(dT);
