...
data = map.boundary;
marg = 1.0; % 0.25 -> (CJ) this is just map size
...
K2_tprime = (90/12)*(t_prime^3)/(T^5) - (90/4)*(t_prime^2)/(T^4) + ...
    (30/2)*(t_prime^1)/(T^3);
optParams.K2_tprime = K2_tprime; %(CJ) param
...
optParams.obs = obs;
optParams.map = map;
optParams.max_vel = max_vel; % (CJ) param
...
optParams.d_min = 0.2; %0.2-> min mutual separation % (CJ) param
p_0 = MX.sym('p_0',3,1);
v_0 = MX.sym('v_0',3,1);
