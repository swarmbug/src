function [xx, yy, zz] = ccc_fct_profiling_reach_avoid_Ndrones_varvel(...
    c_max_per_axis, c_max_vel, c_max_accl, c_M1, c_K1_T, c_aa, c_bb, c_cc, ...
    c_tp1, c_tp2, c_t_prime, c_C, c_C1, c_C2, c_Ib, c_Jb, c_Ja, c_Ia, c_da, c_dv) %,  Ja, Ia) %<-3rd step c_da, c_dv)%<-2nd step, 

%% start reach avoid example for a multiquads with eth tracker++
disp('alg: c_max_per_axis, c_max_vel, c_max_accl, c_M1, c_K1_T | c_aa, c_bb, c_cc, c_tp1, c_tp2 | c_t_prime');
fprintf('alg: %f %f %f %f %f | %f %f %f %f %f | %f %f %f %f %f | %f  \n', c_max_per_axis, c_max_vel, c_max_accl, c_M1, c_K1_T, c_aa, c_bb, c_cc, ...
    c_tp1, c_tp2, c_t_prime, c_C, c_C1, c_C2, c_Ib, c_Jb);
...
M1 = c_M1 * (1/2*T^5)*[90 0 -15*T^2;-90*T 0 15*T^3;30*T^2 0 -3*T^4];

%tracker limits
max_per_axis = c_max_per_axis * 1; % (CJ) param
max_vel = c_max_vel * .751; % .751 -> 3 * .751(CJ) param
optParams.max_vel = max_vel;
max_accl = c_max_accl * 1; % (CJ) param
optParams.max_accl = max_accl;
% from vel constraints on pf
K1_T = c_K1_T * ((90/48)*(1/T) - (90/12)*(1/T) +(30/4)*(1/T));
...
bb = c_bb * -(90/2)*(1/T^4);
cc = c_cc * (30/2)*(1/T^3);
tp1 = c_tp1 * (-bb+sqrt(bb^2-4*aa*cc))/(2*aa);
tp2 = c_tp2 * (-bb-sqrt(bb^2-4*aa*cc))/(2*aa);
t_prime = c_t_prime * (tp1*(tp1>=0)*(tp1<=T) + tp2*(tp2>=0)*(tp2<=T)); %pick the right one
...
%     disp(p0);
%     disp(rand(3,N_drones));
    full_N_drones = 12;
    rate_times = 0.5;
    p0 = p0 + rate_times * rand(3,full_N_drones ); %(CJ)
...
%(1/(T^5))*[720 -360*T 60*T^2;-360*T 168*T^2 -24*T^3;...
    %60*T^2 -24*T^3 3*T^4];
da = 0;%1 * c_da;%0;
dv = 0;%1 * c_dv;%0; %start and stop from/at rest
...
prob = struct('f', ccc_fct_profiling_cost_reach_avoid_Ndrones_varvel(var,optParams, c_C, c_C1, c_C2, c_Ib, c_Jb, c_Ja, c_Ia), 'x', var, 'g', g);
...
%         plot3(waypoints{d}(1,:),waypoints{d}(2,:),waypoints{d}(3,:),mar{d});
    ...
    [negative_rob,xx,yy,zz] = ccc_fct_profiling_cost_reach_avoid_Ndrones_varvel(w_opt,optParams, c_C, c_C1, c_C2, c_Ib, c_Jb, c_Ja, c_Ia);
