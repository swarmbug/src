...
if ~isfield(p_swarm, 'd_ref')
    disp('~isfield(p_swarm');
    disp(~isfield(p_swarm, 'd_ref'));
    p_swarm.d_ref = 25; %(CJ) 25 -> 50, 5
end
...
if ~isfield(p_swarm, 'v_ref')
    disp('~isfield(p_swarm');
    disp(~isfield(p_swarm, 'v_ref'));
    p_swarm.v_ref = 6; %(CJ) 6 -> 12, 3
end
...
randomtest = 0;
if randomtest
    p_swarm.Pos0 = p_swarm.P0 + p_swarm.P * [0,1,0; 1,0,0; 1,1,0]'; % this is only for test for fixed starting point
else
    p_swarm.Pos0 = p_swarm.P0 + p_swarm.P * rand(3,p_swarm.nb_agents); %(CJ)
end
...
%%%%%%%%%% (CJ) added
p_swarm.c_r = 0.1;
p_swarm.is_position_written = false;
