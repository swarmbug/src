function [V_command, collisions] = compute_vel_vicsek(self, p_swarm, r_agent, dt)
    ...
    X = self.get_pos_ned(); %이게좌표얻어오는거
    V = self.get_vel_ned(); %이게속도얻어오는거
    ...
    V_command = zeros(3, nb_agents); % Calculate the commanded velocity, 옳지 3개로 되어있구나:xyz렸다!
    ...
    for agent = 1:nb_agents
        ...
        %%%%%%여기가 핵심이구나, v_rep, v_fric, v_obs_v_wall을하나씩조작해서contribution
        %%%%%%score만들 면될 것같다. 오늘 은여기까 지보 고자자
        ...
        %%%%%%여기가 핵심이구나, migration이 뭐하는건진 잘모르겠지만, p_swarm.v_ref가parameter
        %%%%%%로보인다. 속도에 얼마정도 곱해지는걸로 보인다
        ...
        elseif p_swarm.is_active_goal == true
            ...
            % self-propulsion
            if v_norm > 0 % v의 norm값으로 보임..., 방향을 지정해주는걸로 보인다.
