...
# done: check why this function takes 10ms
...
def contribution_leader(robots, simulation_tick, params, OBSTACLES, target_obs):
    ...
    # done: this m_original_new_local_planner is slow (10ms)
...
# done: check why this function takes 40ms
def contribution_others(robots, robot1, leader_follower, arg_target_index, simulation_tick, followers_sp, params, OBSTACLES, target_obs):
    ...
    if leader_follower == 'for_leader':
        ...
        # done: check below block and remove it
        temp_leader_obs_btt = np.linalg.norm(
            after_normal_cal_sp - temp_sp_leader[0])  # new_added
        temp_leader_obs_dia = np.linalg.norm(
            after_normal_cal_sp - temp_sp_leader[1])  # new_added
        ...
        for variable_index in range(0, 4):
            ...
            if variable_index == 0:
                ...
                robots_obstacles_sp_prime = [x for i, x in enumerate(
                    followers_sp_prime + [robot1_sp_prime]) if i != (target_index - 1)]  # done change robot1.sp
                ...
                robots_obstacles_sp_prime = [x for i, x in enumerate(
                    followers_sp_prime + [robot1.sp]) if i != (target_index - 1)]  # done change robot1.sp
                # each drone is defined as a small cube for inter-robots collision avoidance
                robots_obstacles_prime = poses2polygons(
                    robots_obstacles_sp_prime)
                # combine exisiting obstacles on the map with other robots[for each i: i!=p] in formation
...
# done: remove arrays for record, later
...
# checking crash
# done: this will be removed after simply checking
def checker_crash_simple(simulation_tick, _print=False):

    for o_i in range(0, 3):
        # done: activate below code when you test this function
...
def frange(list_x, start, stop, step):
    i = start
    while i < stop:
        # yield i
        i += step
...
def interp(comparison_1, comparison_2):
    # fix this soon :)
    length_standard = len(comparison_1)
    length_current = len(comparison_2)

    fitted_matrix = np.empty([1, length_standard])

    # list_of_ints = list(range(3))
    # comparison_2[:, col_idx]
    temp_x_list = []
    res = frange(temp_x_list, 0, length_standard,
                 (length_current/length_standard))
    num_columns = 9
    for col_idx in range(0, num_columns):
        xp = list(range(length_current))
        fp = comparison_2[:, col_idx]
        fitted_col = np.interp(res, xp, fp)
        fitted_matrix = np.hstack((fitted_matrix, fitted_col))
...
def distance_between_dcc(comparison_1, comparison_2):
    ...
    # fitted_comparison_2 = interp(comparison_1, comparison_2)
    fitted_comparison_2 = comparison_2

    end_tick = 100
    if len(comparison_1) <= len(fitted_comparison_2):
        end_tick = len(fitted_comparison_2)
    else:
        end_tick = len(fitted_comparison_2)
    ...
    for tick_index in range(start_tick, end_tick):
        for object_index in range(0, num_object):
            temp_temp = math.pow((comparison_1[tick_index][object_index] -
                                  fitted_comparison_2[tick_index][object_index]), 2)
